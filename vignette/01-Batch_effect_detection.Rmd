...
```{r}
hd.lmm = lmer(hd.tss.clr[,1]~ hd.batch + 1|hd.trt)
summary(hd.lmm)
```

As the batch $\times$ treatment design of HD data is nested and unbalanced, the linear model is unable to fit. We therefore fit a linear mixed model with treatment (genotype) effect as random effect to obtain the P value of batch (cage) effect. The difference between cages is statistically significant (P < 0.001). 
